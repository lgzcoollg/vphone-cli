...
int get_tss_response(struct idevicerestore_client_t* client, plist_t build_identity, plist_t* tss)
{
	...
	*tss = NULL;
	...
	/* vphone: the only edit to upstream code in this target. An offline
	   restore installs a saved, FULL TSS response here instead of asking
	   Apple; -T/--ticket cannot do that, because it replaces client->tss
	   with a dict holding only ApImg4Ticket and so loses every per-component
	   signature. The hook fires for &client->tss alone and leaves the
	   recovery-OS root ticket request to go out normally. The reasoning, and
	   the line numbers it rests on, are in vphone_restore_bridge.c under
	   "THE OFFLINE TICKET". */
	extern int vphone_offline_tss_lookup(struct idevicerestore_client_t*, plist_t, plist_t*);
	if (vphone_offline_tss_lookup(client, build_identity, tss)) {
		return 0;
	}
