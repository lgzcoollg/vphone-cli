...
/* "/bin/launchctl" + subcommand args (max observed: 3) + NULL */
#define LAUNCHCTL_ARGV_MAX 5
 ...
 * elevated privileges and are run directly. State-changing subcommands act
 * on a system-domain LaunchDaemon and require the caller to have started the
 * restore command as root. This library never prompts or invokes sudo.
 */
