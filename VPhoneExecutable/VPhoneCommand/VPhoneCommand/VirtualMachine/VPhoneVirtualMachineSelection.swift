...
enum VPhoneVirtualMachineSelection {
    ...
    static func resolveNewName(_ provided: String?, prompt: String) throws -> String {
        ...
        guard isTTY else { throw VPhoneVirtualMachinePickerError.notInteractive }
        FileHandle.standardError.write(Data((prompt + " ").utf8))
        guard let line = readLine(strippingNewline: true)?
            .trimmingCharacters(in: .whitespacesAndNewlines), !line.isEmpty
        else {
            throw VPhoneVirtualMachinePickerError.aborted
        }
        return line
    }
}
