...
- Method: focused `KernelJBPatcher.patch_vm_map_protect()` dry-runs, in the Python environment this repository had on that date. Both the Python patcher and that environment are gone; the port is `Sources/FirmwarePatcher/Kernel/JailbreakPatches/`.
...
- Cross-checked against recovered symbols in `research/kernel_symbols/json/kernelcache.research.vphone600.bin.symbols.json`.
